...
public class BasePage {
    ...
    protected String url = "https://uatwebmmh360.mmh-demo.com";
...
//            act.moveByOffset(X, Y).click().build().perform();
    ...
    public static SanityProviderPageContainer pageContainer;
    ...
    // Below Methods is created by Neelakandan.
    public boolean verifyListElement(List<WebElement> element, int a) {
        boolean isVerify = false;
        try {
            if (element.size() > 0) {
                isVerify = element.get(a).isDisplayed();
            }
        } catch (NoSuchElementException error) {
            error.getMessage();
            isVerify = false;
    ...
    public static boolean clickListElement(List<WebElement> element, int b) {
        boolean blResult = false;
        try {
            element.get(b).click();
            blResult = true;
    ...
    protected WebElement waitForListElement(List<WebElement> element, int c) {
        return wait.until(ExpectedConditions.visibilityOf(element.get(c)));
    }


    public void tabHandles(int value, String url) {
        try {
    ...
    public void tabHandle(int value) {
        try {
